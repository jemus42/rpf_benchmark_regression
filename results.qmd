...
```{r}
runtimes <- jobs[!is.na(time.running) & learner_id != "featureless", 
     ...
     by = .(learner_id, task_id)][,
       mean_mem_used := fifelse(is.nan(mem.used), 4096, mem.used)
     ][]
saveRDS(runtimes, here::here("attic", "runtimes.rds"))
```
...
```{r tuning-results-rpf-splits-tab}
...
data.table::rbindlist(
  ...
  summarize(
    ...
    fmt = glue::glue("{median} ({n}, {min_max})"),
    .by = c("task_id", "learner_id")
  ) |>
  arrange(task_id) |>
  left_join(
  ...
  kableExtra::kable(
    caption = "Median (number of replications, min - max) number of tuning evaluations per learner and task. (-) denotes a constant number of evaluations across resampling iterations."
  ) |>
  kableExtra::kable_styling()
```
